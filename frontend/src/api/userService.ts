    ...
    try {
      ...
      while (!currentToken && retries < MAX_RETRIES) {
        try {
          ...
          console.log(`Got token on attempt ${retries + 1}: ${currentToken.substring(0, 10)}...`);
        } catch (tokenError) {
          console.error(`Token retrieval attempt ${retries + 1} failed:`, tokenError);
          retries++;
      ...
      if (!currentToken) {
        console.error("Failed to retrieve authentication token after multiple attempts");
        throw new Error('Failed to obtain authentication token');
      }
      
      // Make the request with explicit token in headers
      console.log(`Making API request with token: ${currentToken.substring(0, 10)}...`);
      ...
      const response = await apiClient.post(`/user/store-user/${firebase_uid}`, filteredUserData, {
        headers: {
