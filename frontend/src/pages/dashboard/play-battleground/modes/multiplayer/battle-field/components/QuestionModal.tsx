...
const QuestionModal: React.FC<QuestionModalProps> = ({
    ...
    const getTimeLimit = (): number => {
        ...
        if (timeReductionEffect) {
            ...
            console.log(`Time manipulation effect applied: ${timeLimit}s from original ${originalTime}s`);
    ...
    const fetchCardEffects = async () => {
        try {
            ...
            console.log(`Checking for card effects targeting ${playerType} (isHost: ${isHost})`);
            ...
            const response = await axios.get(
                `${import.meta.env.VITE_BACKEND_URL}/api/gameplay/battle/card-effects/${sessionUuid}/${playerType}`
            );
            ...
            if (response.data.success && response.data.data.effects) {
                ...
                console.log(`Found ${response.data.data.effects.length} card effects targeting ${playerType}`);

                // Check if there's a time reduction effect
                const timeEffect = response.data.data.effects.find(
                    (effect: CardEffect) => effect.type === 'normal-1' && effect.effect === 'reduce_time'
                );
    ...
    const consumeCardEffect = async (effectType: string) => {
        try {
            ...
            setCardEffects(prevEffects =>
                prevEffects.filter(effect => !(effect.type === effectType && !effect.used))
            );
    ...
    useEffect(() => {
        if (isOpen && timeReductionEffect) {
            ...
            reductionElement.className = 'fixed inset-0 flex items-center justify-center z-[999] bg-black/40';

            const reductionPercent = timeReductionEffect.reduction_percent || 30;
            const originalTime = difficultyMode?.toLowerCase().includes('easy') ? 20 :
                difficultyMode?.toLowerCase().includes('hard') ? 10 : 15;

            // Calculate new time
            const reducedTime = Math.max(Math.floor(originalTime * (1 - reductionPercent / 100)), 5);
    ...
    const renderTimerDisplay = () => {
        ...
        return (
            <div className={`absolute top-4 left-4 px-4 py-2 rounded-md shadow-lg ${timeReductionEffect
                ? 'bg-purple-800 border-2 border-purple-400 animate-pulse'
                : 'bg-gray-800'
                }`}>
                <div className="flex items-center">
                    <span className="text-white font-bold mr-2">Time:</span>
                    <span className={`${isNearZero
                ...
                <div className="absolute top-4 right-4 bg-purple-800 text-white px-4 py-2 rounded-md shadow-lg border-2 border-purple-400">
                    <div className="flex flex-col items-center">
                        ...
                        <span className="text-sm mt-1">
                            Time reduced by <span className="font-bold text-red-400">{reductionPercent}%</span>
                        </span>
                    </div>
                </div>
            );
