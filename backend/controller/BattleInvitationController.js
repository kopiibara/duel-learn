...
// Add this new controller function for creating initial battle lobbies without a guest player
export const createInitialBattleLobby = async (req, res) => {
    try {
        const {
            sender_id,
            sender_username,
        ...
        // Validate required fields
        if (!sender_id || !lobby_code || !question_types) {
            return res.status(400).json({
                success: false,
                message: "Missing required fields: sender_id, lobby_code, or question_types"
            });
        }
        ...
        const query = `
            ...
            (sender_id, sender_username, sender_level, 
             ...
             lobby_code, status, 
             question_types, study_material_title)
            VALUES (?, ?, ?, NULL, 'Pending Player', 1, ?, 'no_invitation', ?, ?)
        `;
        ...
        res.status(201).json({
            success: true,
            message: "Battle lobby created successfully",
            data: {
                id: result.insertId,
                lobby_code,
                question_types
        ...
        console.error('Error creating initial battle lobby:', error);
        res.status(500).json({
            success: false,
            message: "Failed to create initial battle lobby",
            error: error.message
        });
    }
};
