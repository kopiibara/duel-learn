...
import Top1LB from "../assets/images/Top1LB.png";
import Top2LB from "../assets/images/Top2LB.png";
import Top3LB from "../assets/images/Top3LB.png";


const SideListNavi = () => {
  // State to manage dynamic data
  const [covenInvitations, setCovenInvitations] = useState([
    { id: 1, name: "PERALTA08", level: 1, avatar: Profile },
    { id: 2, name: "KOPIBARA", level: 6, avatar: ProfileIcon },
  ]);

  const [covenHierarchy, setCovenHierarchy] = useState([
  ...
  return (
    <>
      ...
      <div className="hidden side-list-navi pr-10 lg:block w-96 sm:w-[28rem] md:w-[32rem] lg:w-[31rem] p-4 flex-shrink-0">
        {/* Coven Invitation */}
        <div
            ...
            {covenHierarchy.map((member, index) => (
              <div key={member.id} className="flex items-center justify-between mb-4">
                <div className="flex items-center">
                  ...
                  {/* Profile Picture */}
                  <div
                    className={`relative ${index >= 3 ? "w-8 h-8" : "w-10 h-10"
                      } mr-3`}
                  >
                    <img
                      src={member.avatar}
